...
# def plot_graphs_for_node(node):
#     # Define a range of weights and probabilities of correct answers
#     weights = np.linspace(0.5, 3, 10)
#     probabilities = np.linspace(0, 1, 10)

#     # Store the original weight and probability_known of the node
#     original_weight = model._node[node]['weight']
#     original_probability_known = model._node[node]['probability_known']

#     # Create a 2D array to store the final probabilities for each combination of weight and probability
#     final_probabilities = np.zeros((len(weights), len(probabilities)))

#     for i, weight in enumerate(weights):
#         for j, prob_correct in enumerate(probabilities):
#             # Update the node's weight and probability_known
#             model._node[node]['weight'] = weight
#             model._node[node]['probability_known'] = prob_correct

#             # Update the CPDs and obtain the final probabilities
#             update_cpd(node, prob_correct)
#             final_probabilities[i, j] = model._node[node]['probability_known']

#     # Restore the original weight and probability_known of the node
#     model._node[node]['weight'] = original_weight
#     model._node[node]['probability_known'] = original_probability_known

#     # Plot the final probabilities for the node
#     plt.figure(figsize=(8, 6))
#     plt.contourf(probabilities, weights, final_probabilities, cmap='viridis', levels=20)
#     plt.colorbar()
#     plt.xlabel('Probability of Correct Answer')
#     plt.ylabel('Weight of the Node')
#     plt.title(f'Final Probabilities for {node}')
#     plt.show()

# # Generate graphs for each node
# for node in model.nodes():
#     plot_graphs_for_node(node)
...
root_node = 'Introduction to Programming'  # Replace this with the identifier of the root node of your tree
